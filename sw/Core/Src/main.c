...
int main(void)
{
  ...
  createMainMCU(&huart1, &hdma_usart1_rx, &hdma_usart1_tx,
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : START_TX_Pin */
  GPIO_InitStruct.Pin = START_TX_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLDOWN;
  HAL_GPIO_Init(START_TX_GPIO_Port, &GPIO_InitStruct);
...
void Error_Handler(void)
{
  ...
  while (1)
  {
    ...
    for(uint32_t i = 0; i < 3200000; i++){ __NOP(); }
    HAL_GPIO_WritePin(TEST_LED_GPIO_Port, TEST_LED_Pin, GPIO_PIN_RESET);
    for(uint32_t i = 0; i < 3200000; i++){ __NOP(); }
  }
